  ...
  notifications: [
    {
      ...
      message: "The local Rooz engine is fully online. Ready to structure and automate your workflows.",
      read: false,
...
IMPORTANT: The user's active interface language is ${isFa ? "Persian (Farsi)" : "English"}. You MUST write all the text values of this JSON object (planTitle, focusProjectName, highImpactTask, morningSession, afternoonSession, eveningReview, logicalRationale) in ${isFa ? "Persian (Farsi)" : "English"}.`;
  ...
  const plan = {
    ...
    logicalRationale: isFa
      ? `امروز تمرکز روی پروژه «${primaryProject ? primaryProject.name : "اصلی"}» خواهد بود. ${blockedTask ? `کار «${blockedTask.title}» به تعویض افتاده و مسدود است، توصیه می‌شود صبح اول به آن بپردازید.` : "کار روی وظایف با اولویت بالا بهترین مسیر پیش روی شماست."}`
...
app.post("/api/ai/agent", async (req, res) => {
  ...
  try {
    ...
    const systemPrompt = `You are an elite, fully autonomous AI Agent operating at the core of a next-generation personal Rooz OS.
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Rooz Daily Operating System server running on port ${PORT}`);
  });
}
